...
meta_genes_raw <- getBM(attributes = c("ensembl_gene_id",
...
meta_genes_tpm <- getBM(attributes = c("ensembl_gene_id",
                                    "ensembl_gene_id_version",
                                     "hgnc_symbol",
                                     "description",
                                  "chromosome_name",
                      ...
                      values = rownames(expr_raw_filtered),
                      mart = ensembl) %>%
    right_join(data.frame(ensembl_gene_id_version = rownames(expr_tpm_filtered)),
               by = "ensembl_gene_id_version") %>%
    distinct(ensembl_gene_id_version, .keep_all = TRUE)
```
...
metagene_subset <- meta_genes_raw[meta_genes_raw$ensembl_gene_id_version %in% rownames(expr_df), ]
...
#expr_with_gene_id <- cbind(expr_df, meta_genes$ensembl_gene_id, meta_genes$hgnc_symbol)
metagene_subset <- meta_genes_tpm[meta_genes_tpm$ensembl_gene_id_version %in% rownames(expr_df_tpm), ]
...
```{r}
#do PCA for the rlog normalized expected counts
library("factoextra")
library("FactoMineR")
...
#exclude the weird one
ds_omit<-ds[,-5]
pca.data <- PCA(t(ds_omit), scale.unit = TRUE, graph = FALSE)
fviz_pca_ind(pca.data, labelsize = 3, repel = TRUE,xlim=c(-200, 200) , ylim= c(-200, 200))+
   theme(text = element_text(size = 7.5),
         axis.title = element_text(size = 7.5),
         axis.text = element_text(size = 7.5)
...
#exclude the weird one
vs_omit<-vs[,-5]
pca.data <- PCA(t(vs_omit), scale.unit = TRUE, graph = FALSE)
fviz_pca_ind(pca.data, labelsize = 3, repel = TRUE,xlim=c(-200, 200) , ylim= c(-200, 200))+
   theme(text = element_text(size = 7.5),
         axis.title = element_text(size = 7.5),
         axis.text = element_text(size = 7.5)
...
```
```{r}
#check the tpm data using PCA
library("factoextra")
library("FactoMineR")
#expr_filtered <- expr[, -3]
pca.data <- PCA(t(expr_tpm_filtered), scale.unit = TRUE, graph = FALSE)
fviz_pca_ind(pca.data, labelsize = 3, repel = TRUE,xlim=c(-200, 200) , ylim= c(-200, 200))+
   theme(text = element_text(size = 7.5),
         axis.title = element_text(size = 7.5),
         axis.text = element_text(size = 7.5)
...
#exclude the weird one
expr_tpm_filtered_omit<-expr_tpm_filtered[,-5]
pca.data <- PCA(t(expr_tpm_filtered_omit), scale.unit = TRUE, graph = FALSE)
fviz_pca_ind(pca.data, labelsize = 3, repel = TRUE,xlim=c(-200, 200) , ylim= c(-200, 200))+
   theme(text = element_text(size = 7.5),
         axis.title = element_text(size = 7.5),
         axis.text = element_text(size = 7.5)
